...
describe("post /my-pokemons/:id/remove", () => {
    it("should answer with status 200 remove pokemon to user", async () => {
        const user = await createUser();
        const token = await createSession(user.id);
        const pokemon = await createPokemon();
        await createUserPokemon(user.id, pokemon.id);
        ...
        const pokemonAdded = await getRepository(UserPokemon).find();
        expect(pokemonAdded.length).toEqual(0);
        expect(response.status).toBe(200);
    });

    it("should answer with status 400 if user does not have pokemon", async () => {
        const user = await createUser();
        const token = await createSession(user.id);
        const pokemon = await createPokemon();
    ...
    it("should answer with status 401 if no token provided", async () => {
        const response = await supertest(app).post(`/my-pokemons/1/remove`);
